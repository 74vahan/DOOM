// ============================================================
// renderer.js — сборка кадра: потолок/пол, стены, спрайты,
// минимапа. Вызывается каждый кадр из game.js.
// ============================================================

const Renderer = {
  ctx: null,
  W: 0,
  H: 0,       // высота 3D-вида (без HUD)
  palette: {}, // цвета стен, разобранные в [r,g,b] для затемнения
  ...
  init(canvas) {
    ...
    // Разбираем hex-цвета стен в числа один раз (быстрее в цикле)
  ...
  // Главная функция: нарисовать весь кадр
  render() {
    const ctx = this.ctx;

    // --- 1. Потолок и пол (простые прямоугольники) ---
  ...
  // Спрайты рисуем вертикальными полосками шириной 1 пиксель,
  // сравнивая каждую полоску с zbuffer: если стена ближе — полоска
  // не рисуется. Так враги правильно прячутся за углами.
  drawSprites(zbuffer) {
    const ctx = this.ctx;
    ...
    for (const s of items) {
      if (Math.abs(s.rel) > CONFIG.FOV / 2 + 0.4) continue; // за пределами обзора
      const perp = s.dist * Math.cos(s.rel);
      if (perp < 0.2) continue; // вплотную к камере — не рисуем

      const unit = Raycaster.projDist / perp;          // размер 1 игровой единицы в пикселях
      const h = unit * s.worldH;                       // высота спрайта на экране
      const w = h * (s.img.width / s.img.height);      // ширина с сохранением пропорций
      // Экранная X-координата центра спрайта
      const centerX = (Math.tan(s.rel) / halfTan + 1) * 0.5 * this.W;
      const x0 = centerX - w / 2;
      const yBottom = this.H / 2 + unit / 2;           // спрайт стоит на полу
      const y0 = yBottom - h;

      // Лёгкое затемнение дальних спрайтов
      ...
      for (let sx = startX; sx < endX; sx++) {
        if (zbuffer[sx] <= perp) continue; // эта полоска за стеной
  ...
  // Мини-карта в левом верхнем углу
  drawMinimap() {
    const ctx = this.ctx;
    const scale = 3; // пикселей на клетку
    const mx = 6, my = 6;
    ...
    for (let y = 0; y < GameMap.H; y++) {
      for (let x = 0; x < GameMap.W; x++) {
        ...
        if (t === 0) continue;
        ctx.fillStyle = CONFIG.WALL_COLORS[t][0];
    ...
    // Игрок — белая точка + направление взгляда
